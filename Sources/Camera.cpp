...
void Camera::moveForwardGrounded( GLfloat distance ){
	float y = forward.y;
	forward.y = 0;
...
void Camera::moveRightGrounded( GLfloat distance ){
	float y = right.y;
	right.y = 0;
	moveRight( distance );
	right.y = y;
...
void Camera::checkAngle( float &angle ){
...
void Camera::rotateFPS_OX( float angle ){
	...
	projectedForward = projectedForward * cos( angle ) - projectedUp * sin( angle ); // from OZ towrads OX
...
void Camera::rotateFPS_OY( float angle ){
	...
	projectedForward = projectedForward * cos( angle ) + projectedRight * sin( angle ); // from OZ towrads OX
	projectedForward = projectedForward.normalize() * projectedForwardLength;
	projectedForward.y = forward.y;
	forward = projectedForward.normalize();
	right = ( projectedRight * cos( angle ) - projectedForward * sin( angle ) ).normalize();
	up = right.CrossProduct( forward ).normalize();
}

void Camera::rotateTPS_OX( float angle, float distance )
{
	moveForward( distance );
	rotateFPS_OX( angle );
	moveForward( -distance );
...
void Camera::rotateTPS_OY( float angle, float distance )
{
	moveForward( distance );
	rotateFPS_OY( angle );
	moveForward( -distance );
