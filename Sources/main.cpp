...
void updateWindText(){
	if( hair->wind ){
		if( hair->windDirection == X ){
			windText = std::string( "Wind:                         Blowing from X\n" );
			return;
		}
		else if( hair->windDirection == MinusX ){
			windText = std::string( "Wind:                         Blowing from MinusX\n" );
			return;
		}
		else if( hair->windDirection == Z ){
			windText = std::string( "Wind:                         Blowing from Z\n" );
			return;
		}
		else if( hair->windDirection == MinusZ ){
			windText = std::string( "Wind:                         Blowing from MinusZ\n" );
			return;
		}
	}
	else{
...
std::string limitDecimalsTo2( std::string text ){
	std::size_t whereAt;
	whereAt = text.find( "." );
...
void drawText( std::string textToBeDisplayed ){
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			glColor3f( 1, 1, 1 );
			glutStrokeString( GLUT_STROKE_MONO_ROMAN, (const unsigned char *)textToBeDisplayed.c_str() );
		glPopMatrix();
		glMatrixMode( GL_PROJECTION );
	glPopMatrix();
}
...
void reshape( int width, int height ){
	...
	glMatrixMode( GL_PROJECTION );
	glLoadIdentity();
	gluPerspective( FIELD_OF_VIEW_ANGLE, (GLdouble)width / (GLdouble)height, Z_NEAR, Z_FAR );
}
...
void keyboard( unsigned char key , int x, int y ){
	...
	case KEY_G_CAPITAL:
	case KEY_G_LOWER:
		hair->gravity = ( N_GRAVITY_TYPES - hair->gravity ) % N_GRAVITY_TYPES; // Does not work for N_GRAVITY_TYPES > 3.
		updateGravityText();
		break;
	case KEY_H_CAPITAL:
	case KEY_H_LOWER:
...
void idle(){
	...
	if( iteration % MEASUREMENT_STEP == 0 ){
		finish *= 1000000.0f; // Modify here for different order of magnitude. Current: us
		stream_finish << finish;
		measurementOverheadTime =  "Measurement overhead time:    " + limitDecimalsTo2( stream_finish.str() ) + " us\n";
	}

	// On CPU:
	if( RUNS_ON == "CPU" ){
		startCounter( CPU_PHYSICS_TIME );
		hair->timeSinceLastSimulation =	1000 * getCounter( INTER_SIMULATION_TIME );
		hair->simulateAnimation();
		startCounter( INTER_SIMULATION_TIME );
		if( iteration % MEASUREMENT_STEP == 0 ){
			finish = getCounter( CPU_PHYSICS_TIME );
			finish *= 1000.0f; // Modify here for different order of magnitude. Current: ms
			flops = hair->vertices.size() * ( 4 + 8 * hair->length / ( hair->length + 1 ) ) / finish / 1000.0f; // Modify here for different order of magnitude. Current: GFLOPS
			stream_finish << finish;
			stream_flops << flops;
			cpuPhysicsTime =  "CPU physics time:             " + limitDecimalsTo2( stream_finish.str() ) + " ms\nRaw performance:              " + limitDecimalsTo2( stream_flops.str() ) + " GFLOPS\n";
		}
	}
	...
	// On GPU:
		...
		if( cudaStatus != cudaSuccess )
			errorHandler( cudaStatus, "cudaGraphicsMapResources() failed.", cudaGetErrorString( cudaStatus ) );

		cudaStatus =  cudaGraphicsResourceGetMappedPointer( (void**)&dev_vertices, &size, cudaVBO );
		if( cudaStatus != cudaSuccess )
			errorHandler( cudaStatus, "cudaGraphicsResourceGetMappedPointer() failed.", cudaGetErrorString( cudaStatus ) );

		size /= sizeof( Vector3D ); // Convert from number of bytes to number of elements.
		hair->timeSinceLastSimulation =	1000 * getCounter( INTER_SIMULATION_TIME );
		cudaStatus = kernelWrapper( dev_vertices, size, hair->length, hair->gravity, hair->centerOfGravity, hair->upsizedCenterOfGravity, hair->downsizedCenterOfGravity, hair->windDirection, hair->wind, FALL_SPEED, WIND_SPEED * (float)( rand() % 100 ) / 100.0f, kernelTime, hair->timeSinceLastSimulation );
		startCounter( INTER_SIMULATION_TIME );
		kernelTime = WIND_SPEED * (float)( rand() % 100 ) / 100.0f;
		if( cudaStatus != cudaSuccess )
			errorHandler( cudaStatus, "kernelWrapper() failed.", cudaGetErrorString( cudaStatus ) );
		cudaStatus = tiltKernelWrapper( dev_vertices, size, tilt );
		if( cudaStatus != cudaSuccess )
			errorHandler( cudaStatus, "tiltKernelWrapper() failed.", cudaGetErrorString( cudaStatus ) );
		tilt = 0;
		...
		if( iteration % MEASUREMENT_STEP == 0 ){
			...
			kernelTime *= 1.00f; // Modify here for different order of magnitude. Current: ms
			kernelFlops = hair->vertices.size() * ( 4 + 8 * hair->length / ( hair->length + 1 ) ) / kernelTime / 1000.0f; // Modify here for different order of magnitude. Current: GFLOPS
			stream_kernelTime << kernelTime;
			stream_kernelFlops << kernelFlops;
			cudaTime =  "CUDA measured physics time:   " + limitDecimalsTo2( stream_kernelTime.str() ) + " ms\nRaw performance:              " + stream_kernelFlops.str() + " GFLOPS\n";
			// GPU Physics Time
			finish *= 1000.0f; // Modify here for different order of magnitude. Current: ms
			flops = hair->vertices.size() * ( 4 + 8 * hair->length / ( hair->length + 1 ) ) / finish / 1000.0f; // Modify here for different order of magnitude. Current: GFLOPS
			stream_finish << finish;
			stream_flops << flops;
			gpuPhysicsTime =  "GPU physics time:             " + limitDecimalsTo2( stream_finish.str() ) + " ms\nRaw performance:              " + limitDecimalsTo2( stream_flops.str() ) + " GFLOPS\n";
		}
	}
...
void display(){
	...
	if( iteration % MEASUREMENT_STEP == 0 ){
		...
		stream_finish << finish;
		stream_fps << fps;
		frameTime =  "Frame time:                   " + limitDecimalsTo2( stream_finish.str() ) + " ms\nFrame rate:                   " + limitDecimalsTo2( stream_fps.str() ) + " fps\n";
	}
...
void initScene(){
	...
	// Declarations.
	std::ostringstream nOfHairVerticesStream, nOfBaseHairVerticesStream, hairLengthStream;

	cudaStatus = cudaGraphicsUnregisterResource( cudaVBO );
//	if( cudaStatus != cudaSuccess )
//		errorHandler( cudaStatus, "cudaGraphicsUnregisterResource() failed.", cudaGetErrorString( cudaStatus ) );
	...
	cudaMalloc( (void **)&dev_vertices, hair->vertices.size() * sizeof( Vector3D ) );

	cudaStatus = cudaGraphicsGLRegisterBuffer( &cudaVBO, hair->vbo, cudaGraphicsRegisterFlagsNone );
	if( cudaStatus != cudaSuccess )
		errorHandler( cudaStatus, "cudaGraphicsGLRegisterBuffer() failed.", cudaGetErrorString( cudaStatus ) );
...
void initGL(){
	...
	if( cudaStatus != cudaSuccess )
		errorHandler( cudaStatus, "cudaGetDeviceCount() failed. Do you have a CUDA-capable GPU installed?", cudaGetErrorString( cudaStatus ) );
	std::cout << count << " CUDA device" << ( count == 1 ? "" : "s" ) << " available." << std::endl;
	for( i = 0; i < count; i += 1 ){
	...
	if( cudaStatus != cudaSuccess )
		errorHandler( cudaStatus, "cudaSetDevice() failed.", cudaGetErrorString( cudaStatus ) );

	// Display CUDA driver version.
	cudaResult = cuDeviceGet( &cudaDevice, 0 );
	if( cudaResult != CUDA_SUCCESS )
		errorHandler( cudaResult, "cuDeviceGet() failed.", cuGetErrorString( cudaResult ) );

	// Initialize CUDA driver API.
	cudaResult = cuInit( 0 );
	if( cudaResult != CUDA_SUCCESS )
		errorHandler( cudaResult, "cuInit() failed.", cuGetErrorString( cudaResult ) );

	// Display CUDA driver version.
	cudaResult = cuDriverGetVersion( &driverVersion );
	if( cudaResult != CUDA_SUCCESS )
		errorHandler( cudaResult, "cuDriverGetVersion() failed.", cuGetErrorString( cudaResult ) );
	...
	if( cudaStatus != cudaSuccess )
		errorHandler( cudaStatus, "cudaDeviceReset() failed.", cudaGetErrorString( cudaStatus ) );
	// Record the curent thread as CUDA-interoperable.
	cudaStatus = cudaGLSetGLDevice( 0 );
	if( cudaStatus != cudaSuccess )
		errorHandler( cudaStatus, "cudaGLSetGLDevice() failed.", cudaGetErrorString( cudaStatus ) );
	...
	cudaResult = cuGLCtxCreate( &cudaContext, CU_CTX_LMEM_RESIZE_TO_MAX, cudaDevice );
	if( cudaResult != CUDA_SUCCESS )
		errorHandler( cudaResult, "cuGLCtxCreate() failed.", cuGetErrorString( cudaResult ) );
	//*/
	...
	glViewport( (GLint)0, (GLint)0, (GLsizei)WINDOW_WIDTH, (GLsizei)WINDOW_HEIGHT );

	glMatrixMode( GL_PROJECTION );
	glLoadIdentity();
	gluPerspective( FIELD_OF_VIEW_ANGLE, (GLdouble)WINDOW_WIDTH / (GLdouble)WINDOW_HEIGHT, Z_NEAR, Z_FAR );
	glMatrixMode( GL_MODELVIEW );
	glLoadIdentity();
...
void init(){
	...
	if( cudaStatus != cudaSuccess ){
		errorHandler( cudaStatus, "main()/cudaDeviceSynchronize() failed.", cudaGetErrorString( cudaStatus ) );
	}
}
