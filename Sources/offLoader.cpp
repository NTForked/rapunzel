...
void offLoader::readOFF( const std::string filename ){
	...
	if( type == StraightForward ){
		...
		for( i = 0; i < nOfFaces; i += 1 ){
			normal = computeNormal( vertices[faces[3 * i]], vertices[faces[3 * i + 1]], vertices[faces[3 * i + 2]] );
		...
		for( i = 0; i < nOfFaces; i += 1 ){
			...
			normals.push_back( computeNormal( finalVertices[3 * i], finalVertices[3 * i + 1], finalVertices[3 * i + 2] ) );
		}
	}
}
