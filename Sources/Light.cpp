...
void Light::render( Vector3D position, Vector3D direction ){ // Placing light in scene.
	...
	glLightfv( GL_LIGHT0 + id, GL_POSITION, Vector4D( position.x, position.y, position.z, 0 ).Array() );
	//> Ideal.
	//glLightfv( GL_LIGHT0 + id , GL_SPOT_DIRECTION, Vector3D( 0, 0, 0 ).Array() );
	//> Spot.
	//glLightfv( GL_LIGHT0 + id , GL_SPOT_DIRECTION, direction.normalize().Array() );
