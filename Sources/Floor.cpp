...
GLuint Floor::loadBMP( const std::string textureFile ){
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, bmp.width, bmp.height, 0, GL_RGB, GL_UNSIGNED_BYTE, data.c_str() );
...
//	glNormalPointer( GL_FLOAT, offsetof( Mannequin, normals ), NULL );
	...
	unsigned int i;
	for( i = 0; i < 6; i += 1 ){
		glNormal3f( normals[indices[4 * i]].x, normals[indices[4 * i]].y, normals[indices[4 * i]].z );
		glTexCoord2f( textures[0].x, textures[0].y );
		glVertex3f( vertices[indices[4 * i]].x, vertices[indices[4 * i]].y, vertices[indices[4 * i]].z );
		glTexCoord2f( textures[1].x, textures[1].y );
		glVertex3f( vertices[indices[4 * i + 1]].x, vertices[indices[4 * i + 1]].y, vertices[indices[4 * i + 1]].z );
		glTexCoord2f( textures[2].x, textures[2].y );
		glVertex3f( vertices[indices[4 * i + 2]].x, vertices[indices[4 * i + 2]].y, vertices[indices[4 * i + 2]].z );
		glTexCoord2f( textures[3].x, textures[3].y );
		glVertex3f( vertices[indices[4 * i + 3]].x, vertices[indices[4 * i + 3]].y, vertices[indices[4 * i + 3]].z );
	}
	glEnd();
	glBindTexture( GL_TEXTURE_2D, 0 );
	//*/
