...
Hair::Hair(HairType _type,
    ...
    for (i = 0; i < vertices.size(); i += 1) {
        ...
        if (remainder != 0) {
            vertices[i] =
                vertices[i - 1] +
                (vertices[i - remainder] - centerOfGravity).normalize() *
                    HAIR_STRAND_LENGTH;
        }
    }
    ...
    for (i = 0; i < vertices.size(); i += 1)
        if (i % (length + 1) == 0) {
            distancesUpToCenter[i / (length + 1)] =
                vertices[i].Distance(centerOfGravity) +
                COLLISION_PADDING_OFFSET;
        }
...
void
Hair::tiltForward(float _rotation) {
    // Declarations.
    unsigned int i;
...
void
Hair::tiltSideways(float _rotation) {
    // Declarations.
    unsigned int i;
...
Hair::computeCustomDraw() {
    ...
    for (i = 0; i < vertices.size(); i += 1) {
        for (l = 1; l < length; l += 1) {
            finalVertices(i, l) =
                getNextVertex(finalVertices(i, l - 1), centerOfGravity,
                              greatestY + HAIR_HEIGHT, l);
        }
    }
    for (i = 0; i < finalVertices.size() / length; i += 3) {
        for (l = 1; l < length; l += 1) {
            ...
            finalNormals.push_back( computeNormal( finalVertices( i, l ),
            finalVertices( i + 1, l ), finalVertices( i + 2, l ) ) );
            //*/
            aux.push_back(finalVertices(i, l));
            ...
            finalNormals.push_back(computeNormal(finalVertices(i, l),
                                                 finalVertices(i, l - 1),
                                                 finalVertices(i + 2, l - 1)));

            aux.push_back(finalVertices(i + 2, l));
            aux.push_back(finalVertices(i, l));
            aux.push_back(finalVertices(i + 2, l));
            finalNormals.push_back(computeNormal(finalVertices(i + 2, l),
                                                 finalVertices(i, l),
                                                 finalVertices(i + 2, l)));
            ...
            finalNormals.push_back(computeNormal(finalVertices(i + 2, l),
                                                 finalVertices(i + 2, l - 1),
                                                 finalVertices(i + 1, l - 1)));

            aux.push_back(finalVertices(i + 1, l));
            aux.push_back(finalVertices(i + 2, l));
            aux.push_back(finalVertices(i + 1, l - 1));
            finalNormals.push_back(computeNormal(finalVertices(i + 1, l),
                                                 finalVertices(i + 2, l),
                                                 finalVertices(i + 1, l - 1)));
...
Hair::draw(Vector3D color) {
    ...
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT,
                 Vector4D(color.x, color.y, color.z, 1).Array());
    // glMaterialfv( GL_FRONT_AND_BACK, GL_SPECULAR, Vector4D( HAIR_COLOR, 1
    // ).Array() );
...
float
Hair::collisionOffset(Vector3D vertex, Vector3D referenceVertex) {
    return referenceVertex.Distance(centerOfGravity) -
           vertex.Distance(centerOfGravity);
}

float
Hair::collisionOffsetAfterFall(Vector3D vertex,
                               Vector3D referenceVertex,
                               float fallSpeed) {
    return referenceVertex.Distance(centerOfGravity) -
           (vertex - Vector3D(0.0f, fallSpeed, 0.0f)).Distance(centerOfGravity);
}

float
Hair::collisionOffsetAfterWind(Vector3D vertex,
                               Vector3D referenceVertex,
                               float windSpeed) {
    return referenceVertex.Distance(centerOfGravity) -
           (vertex + windDirection * windSpeed).Distance(centerOfGravity);
}

void
Hair::restoreFromCollision(Vector3D &vertex,
    ...
    if (collisionOffset_v > 0) {
        vertex += (vertex - centerOfGravity).normalize() *
                  (collisionOffset_v + RESTORATION_OFFSET);
    }

    // If real inter-node hair length surpasses than permitted length
    if (vertex.Distance(previousVertex) >
        HAIR_STRAND_LENGTH * HAIR_ELASTICITY_FACTOR) {
        // Restore it to normal length.
        vertex = previousVertex +
                 (vertex - previousVertex).normalize() * HAIR_STRAND_LENGTH;
    }
}

void
Hair::fall(Vector3D &vertex,
           Vector3D copy,
           Vector3D referenceVertex,
           float fallSpeed) {
    // If it's standing up
...
void
Hair::negativeGravity(Vector3D &vertex,
                      Vector3D copy,
                      Vector3D referenceVertex,
                      float fallSpeed) {
    // If it's hanging down
...
Hair::blowWind(Vector3D &vertex,
    ...
    // vertex = ( previousVertex - previousPreviousVertex + windDirection * t *
    // t ) * windSpeed;
}
    ...
    for (i = 1; i < vertices.size(); i += 1) {
        ...
        if (remainder != 0) {  // if it's not a base hair
            if (gravity == NORMAL_GRAVITY)
                ...
                negativeGravity(vertices[i], copy[i], copy[i - remainder],
                                FALL_SPEED);
            if (wind) {
                //                delta += DELTA_INCREMENT;
                blowWind(vertices[i], vertices[i - 1], copy[i - remainder],
                         WIND_SPEED * (float)(rand() % 100) / 100.0f,
                         timeSinceLastSimulation, 0);
                //                if( stuck( vertices[i], previousVertices[i] )
                //                )
                //                    vertices[i] += ( vertices[i] -
                //                    centerOfGravity ).normalize() * 0.01f;
            }
            restoreFromCollision(vertices[i], vertices[i - 1],
                                 vertices[i - remainder]);
            //    previousVertices[i] = vertices[i];
        }
    }
}
...
bool
Hair::stuck(Vector3D vertex, Vector3D previousVertex) {
