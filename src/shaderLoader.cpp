//-------------------------------------------------------------------------------------------------
// Descriere: header in care sunt implementate functii pentru incarcare de
// texturi din fisiere BMP
//
// Note:
//        loadShader        - functia pe care o veti folosi pentru a incarca un
//        shader din mai multe fisiere
//---
//        _createShader    - functie ce creaza un obiect shader dintr-un fisier
//        _createProgram    - functie ce creaza un program opengl din mai multe
//        obiecte shader
//
// Nota2:
//        un program shader include mai multe obiecte de tip shader, pentru
//        fiecare din etapele
...
_createShader(const std::string &shader_file, GLenum shader_type) {
    ...
    if (!file.good()) {
        std::cout << "Shader Loader: Nu am gasit fisierul shader "
                  << shader_file << " sau nu am drepturile sa il deschid!"
                  << std::endl;
        std::terminate();
    }
    ...
    if (compile_result == GL_FALSE) {
        ...
        glGetShaderInfoLog(gl_shader_object, info_log_length, NULL,
                           &shader_log[0]);
        std::cout << "Shader Loader: EROARE DE COMPILARE pentru "
                  << str_shader_type << std::endl
...
_createProgram(const std::vector<unsigned int> &shader_objects) {
    ...
    for (std::vector<unsigned int>::const_iterator it = shader_objects.begin();
         it != shader_objects.end(); it++)
    ...
    if (link_result == GL_FALSE) {
        ...
        glGetProgramInfoLog(gl_program_object, info_log_length, NULL,
                            &program_log[0]);
        std::cout << "Shader Loader : EROARE DE LINKARE" << std::endl
                  << &program_log[0] << std::endl;
        return 0;
    }

    // delete the shader objects because we do not need them any more (this
    // should be re-written if usage of separate shader objects is intended!)
    for (std::vector<unsigned int>::const_iterator it = shader_objects.begin();
         it != shader_objects.end(); it++)
        glDeleteShader((*it));
